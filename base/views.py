...
def registerPage(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(request, 'An error ocured during registation !!!')
...
def home(request):
    ...
    room_massages = Massage.objects.filter(
...
def userProfile(request, pk):
    ...
    context = {
        'user': user, 'rooms': rooms, 'room_massages': room_massages, 'topics': topics
    }
...
def updateRoom(request, pk):
    ...
    if request.user != room.host:
        return HttpResponse('You are not allowed here!! You are not a valid user')
